    ...
    <div className="w-full z-50 flex relative items-center justify-center px-4 md:px-10">
      <nav className="flex overflow-hidden py-6 md:py-4 w-full justify-between items-center">
        <div>
          <img
            ...
            alt="Sitemark Logo"
            className="w-20 h-auto md:w-24 lg:w-28 object-contain cursor-pointer"
            onClick={() => navigateTo("landing")}
          />
        </div>
        <div className="flex items-center gap-5">
          <Button content="Sign up" plain onClick={() => navigateTo("dashboard")} />
          <div className="lg:block hidden">
            <Button content="Explore Services" secondary onClick={() => navigateTo("dashboard")} />
          </div>
          <div
      ...
      >
        <div className="w-full flex-col flex-1 h-full justify-between p-4 flex">
          ...
          <Button content="Explore Services" secondary widthFull onClick={() => navigateTo("dashboard")} />
        </div>
      </motion.div>
    </div>
  );
